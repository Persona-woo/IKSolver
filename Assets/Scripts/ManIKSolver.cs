...
public class ManIKSolver : MonoBehaviour
{
    ...
    [Tooltip("Match end effector rotation to target rotation.")]
    ...
    void DoIK(int idx)
    {
        ...
        for (int i = 0; i < boneTransforms.Length; i++)
        {
            if (i == endIndex)
            {
                // Match end effector rotation to target rotation
                if (matchTargetRotation)
                {
                    boneTransforms[i].rotation = target.rotation;
                }
